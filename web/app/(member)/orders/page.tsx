...
export default function OrdersPage() {
  const [state, setState] = useState<
    { status: "loading" } | { status: "error"; message: string } | { status: "loaded"; orders: MemberOrder[] }
  >({ status: "loading" });
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      <h1 className="text-2xl font-semibold tracking-tight text-cashmere-text">My Orders</h1>

      {state.status === "loading" && <p className="text-cashmere-text-muted">Loading your orders…</p>}
      ...
      {state.status === "loaded" && state.orders.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-cashmere-border bg-white px-6 py-16 text-center">
          <Package size={28} strokeWidth={1.5} className="text-cashmere-text-muted" />
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-cashmere-border">
              {state.orders.map((order) => (
                <tr key={order.id}>
                  <td className="px-6 py-4 font-medium text-cashmere-text">{order.orderNumber}</td>
                  <td className="px-6 py-4 text-cashmere-text-muted">
                  ...
                  <td className="px-6 py-4 text-cashmere-text-muted">{order.status}</td>
                  <td className="px-6 py-4 text-right font-medium text-cashmere-text">
