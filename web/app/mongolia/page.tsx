...
//
// 2026-09-11 — Photo Archive was built (submission + moderation) and then
// deliberately pulled back out the same day: the client confirmed it
// should wait for post-launch, since member photo submissions need real
// consent/usage-rights handling first, which this pass never built. The
// backend (MongoliaService.submitPhoto/findApprovedPhotosForMember, the
// MongoliaPhoto model) and the staff moderation page
// (/staff/mongolia/photos) stay as they are — real, tested, harmless to
// leave in place — but the member-facing entry point (the tile below and
// the submit/browse section that used to sit under #photos) is removed,
// since that's the actual consent risk. See PROJECT_TRACKER.md. The bottom
// tab bar shown in the client's mockup (Home/Explore/Vote/Offers/Profile)
// is a separate, still-deferred piece, not attempted here either.
