...
- `api/src/members/`: `MembersService.findOrCreateFromIdentity()` — new members default to the free NEWSLETTER tier; nothing in the auth flow ever grants paid/Founding status (that only happens via the Shopify order webhook in Milestone 4, per the client's "no manual approval, activation on payment" rule). `GET /members/me/orders` returns the current member's `MemberOrderCache` rows, newest first. `GET /members/me/collection` (2026-09-03) flattens `MemberOrderCache.lineItems` (a JSON column, populated by the order-sync webhook) across all of a member's orders — feeds the My Collection page, text-only since real Shopify line items don't include a product image. `GET`/`POST /members/me/data-requests` (2026-09-04) — self-service GDPR access requests, idempotent against an existing pending one; feeds the Settings page. `GET`/`POST`/`DELETE /members/me/wishlist` (2026-09-05, `src/wishlist/`) — backed by the new `WishlistItem` Prisma model; idempotent on (memberId, shopifyProductId), ownership-checked on delete. This is our side of the client's confirmed Wishlist flow only — the actual storefront "Add to Wishlist" click isn't wired up yet (separate integration design question). `GET /members` and `GET /members/:id` (2026-09-07, staff-facing, Club Manager or Member Support) — search/list and a combined detail snapshot for the Members & Users admin view; required importing `StaffModule` into `MembersModule` (a real DI gap the e2e suite caught immediately — `RolesGuard` needs `StaffService`, which wasn't available here before).
- `api/src/staff/`: RBAC. `RolesGuard` + `@Roles('Club Manager', ...)` decorator, checked against `StaffRoleAssignment`; Super Administrator bypasses any role list. **`StaffJwtStrategy`/`StaffAuthGuard` are explicitly provisional** — a JWT-in-httpOnly-cookie placeholder (separate secret from member sessions) standing in until the Milestone 1 auth evaluation picks the real staff login method (magic link, Auth0/Cognito, Passkeys). Nothing above that layer changes when it's swapped. **Verified live**: no session → 401, allowed role → 200, disallowed role → 403, Super Administrator → 200 regardless of the route's role list. `scripts/create-test-staff.ts` mints a staff session token for manual testing until real staff login exists. Also has `POST`/`DELETE /staff/:staffUserId/roles(/:roleName)` (Super Administrator only) to grant/revoke roles. `GET`/`POST /staff` (2026-09-04, Milestone 5's first piece) — directory listing and account creation, Super Administrator only; creation deliberately doesn't grant a role (stays a separate step). `GET /staff/roles` (2026-09-05, Super Administrator only) — lists all 10 seeded roles, feeds the admin UI's role-grant dropdown. `StaffService.findById` (2026-09-05) now includes role names in its return shape — previously didn't, which meant `GET /staff/me` returned no `roles` field at all and crashed the frontend's role-gating logic; caught building the admin UI, fixed, and locked in with a new e2e test.
...
- `api/src/data-subject-requests/`: staff-facing GDPR request queue — `GET /data-subject-requests/pending` (oldest first) and `PATCH /data-subject-requests/:id/complete`, gated to the Member Support role. Completing writes an audit log entry. Deletion requests never appear here — those go straight through `customers/redact` (below), which deletes the Member outright. **Verified for real**: `test/data-subject-requests.e2e-spec.ts` against a live DB, plus live HTTP checks (401/403/200 by role, 404 on an unknown id, 409 on double-complete). Also exposes `createFromMember`/`findForMember` (2026-09-04), used by the member-facing self-service endpoints below — the first non-webhook way a `DataSubjectRequest` gets created.
...
- `web`: Next.js 16 (App Router, TypeScript, Tailwind). `output: "standalone"` in `next.config.ts` — verified the standalone build actually boots and serves traffic, not just that `next build` succeeds.
  - `app/page.tsx` + `app/session-status.tsx`: the sign-in entry point, redesigned 2026-09-05 to match the client's `login-ui.jpeg` (navy/cream marketing layout, monogram, trust rows, footer band — no real photography yet, navy gradient in its place). Checks `GET /members/me` on load; shows "Sign in with Shopify" if signed out, redirects to `/dashboard` if already signed in.
  - `app/(member)/`: the actual member portal, route-grouped so every page under it shares one layout. `layout.tsx` does the auth check once (redirects to `/` if no session), provides the member via `MemberContext` (`contexts/member-context.tsx`), and renders the sidebar (`components/sidebar.tsx`) + header (`components/header.tsx`). `dashboard/page.tsx` (redesigned 2026-09-05, rebuilt same day to match `founding-member-dashboard.jpeg` section-for-section) — full dashboard for Founding/Annual: hero, a benefits row (real Member since/ID + the client's actual confirmed My Benefits copy, per-tier), a Next Release placeholder + real order-count panel (`GET /members/me/orders`), News/Behind-the-Cashmere placeholders, and a 4-item utility row (all four — Exclusive Collections/Members Events/My Benefits/Update Profile — are real links now). Uses `Playfair_Display` (serif, `next/font/google`) for headings, set up in `app/layout.tsx` / `globals.css` alongside the existing Geist sans body font; a different, simplified "Home" view for Newsletter/Mongolia ending in a "Become a Member" section, per the client's spec. `profile/page.tsx` — contact details (read-only, Shopify stays authoritative) + membership detail fields, full access for every tier. `orders/page.tsx` — calls `GET /members/me/orders`; real empty state until orders sync in, real table once they do (order-sync webhook exists, Milestone 4). `member-offers/page.tsx` — a minimal real placeholder, not the actual feature (Milestone 5), built specifically to prove the access-control mechanism below on a real route. `collection/page.tsx` (2026-09-03) — the real feature, not a placeholder: reads `GET /members/me/collection`, wrapped in `RequireAccess area="myCollection"` (Newsletter/Mongolia have no access per the spec). `settings/page.tsx` (2026-09-04) — also a real feature, not a placeholder: a self-service "Request my data" button (GDPR), showing a persisted pending/idempotent state rather than inventing settings toggles with no spec behind them. `benefits/page.tsx` (2026-09-05) — real content, not a placeholder: the client's confirmed My Benefits table, hardcoded per tier (Founding/Annual); Newsletter/Mongolia get an "Invitation to become a member" per the spec's "preview" access level, never the real list. The client's stated goal of an admin-editable `Benefit` model isn't built yet — this hardcodes the confirmed table so the content is live now. `care-repair/page.tsx` (2026-09-05) — the client's confirmed topic list (washing, storage, pilling, simple repairs, longevity) as structured cards; "structure and templates only for now" per the client's own instruction, so each card shows "Guide coming soon" rather than invented care advice. `invitations/page.tsx` (2026-09-05) — the client's own two suggested dummy events ("Mongolia Evening — Meet the Producers", "Meet the Designer"), verbatim; no real registration link exists yet so that button is an honest disabled state, not a fabricated URL; Mongolian-language copy for this event isn't built (no verified translation). `wishlist/page.tsx` (2026-09-05) — a real feature, not a placeholder: reads/writes `GET`/`DELETE /members/me/wishlist`, full access for every tier; stays empty in practice until the CashmereHouse.com "Add to Wishlist" storefront integration exists (separate, unresolved design question). `exclusive-collections/page.tsx` (2026-09-05) — honest placeholder, same as Member Offers: blocked on missing Shopify Storefront API credentials. `help/page.tsx` (2026-09-05) — real per-tier support level (Priority/Standard/General) from the client's spec PDF; links out to CashmereHouse.com for contact since no dedicated support channel has been confirmed yet. `news/page.tsx` (2026-09-07) — honest placeholder, same pattern as Care & Repair; fixes a real pre-existing dead link (both sidebars have always linked to `/news` with no page behind it). `components/bottom-tab-bar.tsx` (2026-09-07) — mobile-only (`md:hidden`), tier-aware 5-item nav matching the client's own mobile mockups (`2.2 Fouder member Mobli.png`/`2.5 CLC MN Founder Mobil.png`), coexisting with the hamburger-triggered full sidebar rather than replacing it; wired into `(member)/layout.tsx` with bottom padding reserved so it never covers page content. **Real photography added 2026-09-08** (`public/images/`, sourced from the client's `/live-images/` batch at the project root — see `PROJECT_TRACKER.md` for the full 25-image inventory): the Dashboard hero (now a real two-column photo layout, not a CSS gradient) and Care & Repair (a real photo banner, previously text-only) both use images that are exact matches for their specific mockup slots. `app/page.tsx` (the pre-login landing page) also now uses the client's real `login-hero.jpeg` — the literal source photo `login-ui.jpeg` was built around — behind the same navy gradient overlay it had before, for the same text contrast.
  - `app/staff/` (2026-09-05, first Milestone 5 admin UI) — separate `/staff/*` app, own `clc_staff_session`-based auth (`layout.tsx`, `contexts/staff-context.tsx`, `components/staff-sidebar.tsx`/`staff-header.tsx`), never sharing state with the member portal. Deliberately no login form for the signed-out state — real staff auth is still a Milestone 1 decision, so this shows an honest placeholder instead of building something throwaway. `staff/directory/page.tsx` (Super Administrator only) — create staff accounts, grant/revoke roles, all on top of already-existing endpoints. `staff/data-requests/page.tsx` (Member Support or Super Administrator) — the GDPR queue, no new backend needed. `staff/members/page.tsx` + `staff/members/[id]/page.tsx` (2026-09-07, Club Manager or Member Support) — search/list members and a combined read-only detail view (profile + orders + collection + wishlist). All role-gated client-side to match the server-side guards, redirecting away before rendering on a direct URL visit by an unauthorized role, same discipline as the member side's `RequireAccess`. `lib/staff-api.ts` holds the fetchers, kept separate from `lib/api.ts`'s member-facing ones on purpose. **`next.config.ts`'s proxy rewrite for `/staff/*` was narrowed 2026-09-07** — it used to be a blanket `/staff/:path*` → API rule left over from before this frontend app existed, which silently swallowed the dynamic `/staff/members/[id]` page (Next's routing order checks non-dynamic pages before `afterFiles` rewrites, but dynamic routes only after — a real, reproduced bug, not theoretical). Now lists only the real backend sub-paths explicitly. One known, NOT-yet-fixed collision remains: bare `GET`/`POST /staff` (the list/create endpoint) shares its path with the frontend's own `/staff` home page — doesn't reproduce in local dev (`NEXT_PUBLIC_API_URL` points straight at `localhost:3000`, bypassing this proxy), but will the moment this is tested through the single-tunnel/production-style setup. See `next.config.ts`'s own comment and `PROJECT_TRACKER.md`.
...
- `api/test/test-app.util.ts` (2026-09-04): every e2e spec file bootstraps its test app through this one shared `createTestApp()` instead of hand-rolling `Test.createTestingModule(...)` each time. Exists because that hand-rolling had drifted from `main.ts`'s real bootstrap — no e2e test had ever registered the global `ValidationPipe`, so DTO validation (`@IsEmail()` etc.) had silently never been exercised by any e2e test, only trusted to work. Found via a genuinely new test that expected a 400 and got a 409 instead. Centralizing this removes the risk of future drift.
