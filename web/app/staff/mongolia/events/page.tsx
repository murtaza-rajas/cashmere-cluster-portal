...
// The two real Mongolia levels (see Member.membershipTier's own comment —
// "Mongolia Founding Member" is tier MONGOLIA, "Mongolia Newsletter" is
// tier NEWSLETTER + region MONGOLIA). FOUNDING/ANNUAL aren't offered here —
// those are international-only tiers, not a real choice on this page.
const MONGOLIA_TIERS: EventInput["tiers"] = ["MONGOLIA", "NEWSLETTER"];
...
const EMPTY_FORM: EventInput = {
  ...
  startsAt: "",
  registrationUrl: "",
  tiers: ["MONGOLIA", "NEWSLETTER"],
  sortOrder: 0,
  active: true,
};

// Mongolia Events — client's explicit instruction (2026-09-11): "Events...
// should follow the same underlying admin logic as the international CLC
// sections." No separate Mongolia event model exists or is needed — Event
// already has a `tiers` field that's included MONGOLIA since the first
// region/tier decision. This is the exact same /event-catalog backend as
// /staff/events, just a Mongolia-filtered view with the tier pre-selected,
// and — deliberately unchanged — the exact same Event Manager role gate as
// the international page, not a watered-down Content-Manager version.
export default function MongoliaEventsAdminPage() {
  const staff = useStaff();
  const router = useRouter();
  const canManage = staffHasAnyRole(staff, ["Event Manager", "Mongolia Editor"]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: EventInput = {
        ...form,
        // Always Mongolia — this page only ever creates/edits Mongolia
        // events, same reasoning as the fixed tier list above.
        regions: ["MONGOLIA"],
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-serif text-3xl tracking-tight text-cashmere-text">Events</h1>
        <p className="mt-1 text-cashmere-text-muted">
          Mongolia events, shown on the Mongolia member home — pick Mongolia Founding Member, Mongolia Newsletter, or
          both per event.
        </p>
      </div>
        ...
        <section className="flex flex-col gap-3">
          {state.rows.length === 0 && (
            <p className="rounded-2xl border border-cashmere-border bg-white p-6 text-center text-sm text-cashmere-text-muted">
              No Mongolia events yet — add one above.
            </p>
          )}
          {state.rows.map((row) => (
