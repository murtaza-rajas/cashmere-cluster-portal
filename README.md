...
- `api/src/members/`: `MembersService.findOrCreateFromIdentity()` — new members default to the free NEWSLETTER tier; nothing in the auth flow ever grants paid/Founding status (that only happens via the Shopify order webhook in Milestone 4, per the client's "no manual approval, activation on payment" rule). `GET /members/me/orders` (new) returns the current member's `MemberOrderCache` rows, newest first — legitimately empty for everyone until the order-sync webhook (Milestone 4) exists to populate that table.
...
- `web`: Next.js 16 (App Router, TypeScript, Tailwind). `output: "standalone"` in `next.config.ts` — verified the standalone build actually boots and serves traffic, not just that `next build` succeeds.
  ...
  - `app/(member)/`: the actual member portal, route-grouped so every page under it shares one layout. `layout.tsx` does the auth check once (redirects to `/` if no session), provides the member via `MemberContext` (`contexts/member-context.tsx`), and renders the sidebar (`components/sidebar.tsx`) + header (`components/header.tsx`). `dashboard/page.tsx` — welcome hero with real data (name, tier, join date, Member ID, Founding status) plus 4 stat cards that honestly show "not tracked yet" rather than invented numbers, since savings/benefits/collection/wishlist all need backend work (Milestone 4/5) that hasn't happened yet. `profile/page.tsx` — contact details (read-only, Shopify stays authoritative) + membership detail fields. `orders/page.tsx` (new) — calls `GET /members/me/orders`; shows a real empty state until the Shopify order-sync webhook exists, a real table (order/date/status/total) once it does. The remaining sidebar links (Member Offers, Wishlist, etc.) exist but 404 — no data model for any of them yet (Milestone 5).
